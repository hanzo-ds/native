	...
	if c.otel {
		newCtx, span := c.tracer.Start(ctx, "Do",
			...
			trace.WithAttributes(
				...
				tracing.ProtocolVersion(c.protocolVersion),
				tracing.QuotaKey(q.QuotaKey),
				tracing.QueryID(q.QueryID),
			),
		)
		m := new(queryMetrics)
		...
		defer func() {
			span.SetAttributes(
				tracing.BlocksSent(m.BlocksSent),
				tracing.BlocksReceived(m.BlocksReceived),
				tracing.RowsReceived(m.RowsReceived),
				tracing.ColumnsReceived(m.ColumnsReceived),
				tracing.Rows(m.Rows),
				tracing.Bytes(m.Bytes),
			)
			if err != nil {
				...
				if errors.As(err, &exc) {
					status = exc.Name
					span.SetAttributes(
						tracing.ErrorCode(int(exc.Code)),
						tracing.ErrorName(exc.Name),
					)
				}
